...
class CoSimulationIDFWriter:
    ...
    def add_cosim_building_properties(
        ...
        """
        assert self.basewriter.zone_data is not None, "make sure that prior to calling add_buidlding_properties attribute zone_data is initialized, e.g. by calling add_buidling_geometry"
        assert (
    ...
    def add_HVAC_template(idf, zone_idf_name, hvac: HVACOperation, name_prefix: str, ureg):
        ...
        outdoor_air_spec_idf_name = CoSimulationIDFWriter.cosimulation_outdoor_air_sepc(idf, hvac.ventilation_fraction_schedule, hvac.outdoor_air_flow_per_zone_floor_area, name_prefix, ureg)
